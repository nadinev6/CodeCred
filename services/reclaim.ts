...
// REMOVED PROBLEMATIC IMPORT: import { ReclaimVerification } from '@reclaimprotocol/inapp-rn-sdk';
...
class ReclaimService {
  ...
  constructor(config: ReclaimConfig) {
    ...
    this.apiBaseUrl = config.apiUrl || 'https://api.reclaim.dev'; // Update with actual API URL
  }
...
const reclaimService = new ReclaimService({
  ...
  appSecret: process.env.EXPO_PUBLIC_RECLAIM_APP_SECRET || '', // Empty as per your config
  apiUrl: process.env.EXPO_PUBLIC_API_BASE_URL || 'https://api.codecred.app',
});
